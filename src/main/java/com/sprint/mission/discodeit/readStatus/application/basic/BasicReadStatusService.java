...
public class BasicReadStatusService implements ReadStatusService {
    ...
    public ReadStatusResponseDto create(ReadStatusCreateRequestDto request) {
        ...
        return ReadStatusResponseDto.from(readStatus.getId(), readStatus.getCreatedAt(), readStatus.getUpdatedAt(),
                readStatus.getUserId(), readStatus.getChannelId(), readStatus.getLastReadTime());
    }
    ...
    public List<ReadStatusResponseDto> findAllByUserId(UUID userId) {
        return readStatusRepository.findAllByUserId(userId).stream()
                .map(readStatus -> ReadStatusResponseDto.from(readStatus.getId(), readStatus.getCreatedAt(), readStatus.getUpdatedAt(),
                        readStatus.getUserId(), readStatus.getChannelId(), readStatus.getLastReadTime()))
                .toList();
    }

    @Override
    public ReadStatusUpdateResponseDto update(UUID id, ReadStatusUpdateRequestDto request) {
        ...
        return ReadStatusUpdateResponseDto.from(readStatus.getId(), readStatus.getCreatedAt(), readStatus.getUpdatedAt(),
                readStatus.getUserId(), readStatus.getChannelId(), readStatus.getLastReadTime());
    }
